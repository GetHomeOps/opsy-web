...
class Notification {
  ...
  static async create(data) {
    const { userId, type = "resource_sent", resourceId, communicationId, title, invitationId, maintenanceRecordId } = data;
    const result = await db.query(
      `INSERT INTO notifications (user_id, type, resource_id, communication_id, title, invitation_id, maintenance_record_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, user_id AS "userId", type, resource_id AS "resourceId", communication_id AS "communicationId", title, invitation_id AS "invitationId", maintenance_record_id AS "maintenanceRecordId", read_at AS "readAt", created_at AS "createdAt"`,
      [userId, type, resourceId || null, communicationId || null, title || null, invitationId || null, maintenanceRecordId || null]
    );
  ...
  static async createForUsers(userIds, data) {
    ...
    for (const userId of userIds) {
      const r = await db.query(
        `INSERT INTO notifications (user_id, type, resource_id, communication_id, title)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, user_id AS "userId", type, resource_id AS "resourceId", communication_id AS "communicationId", title, read_at AS "readAt", created_at AS "createdAt"`,
  ...
  static async listForUser(userId, { limit = 20 } = {}) {
    const result = await db.query(
      `SELECT n.id, n.user_id AS "userId", n.type, n.resource_id AS "resourceId", n.communication_id AS "communicationId", n.title, n.invitation_id AS "invitationId", n.maintenance_record_id AS "maintenanceRecordId", n.read_at AS "readAt", n.created_at AS "createdAt",
