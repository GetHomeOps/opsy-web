...
import { partitionReviewFields } from "../../helpers/documentAnalysisUi";
...
function DocumentAnalysisResultsModal({
  ...
  const { identityFields, systemFields } = useMemo(
    () => partitionReviewFields(reviewFields),
    [reviewFields],
  );
  ...
  const isBid = result?.detectedCategory === "bid";
  const isLoading = status === "loading";
  const canApply = isBid
    ? Boolean(result?.id) && !isLoading && !applying
    : result?.id && selected.size > 0 && !isLoading && !applying;
  ...
  const renderReviewField = (field) => {
    ...
    const destinationLabel = createAsContact
      ? "Installer"
      : field.destination === "property_identity"
        ? "Property identity"
    ...
    const sourceName = field.sourceDocumentName || result.documentName;
    const destinationContext =
      field.destination === "property_identity"
        ? destinationLabel
        : systemLabel
          ? `${systemLabel} → ${destinationLabel}`
          : destinationLabel;
    return (
      <div
        key={field.fieldKey}
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100 truncate">
                {isBid ? "Review quote" : "Review extracted information"}
              </h2>
                ...
                <div className="space-y-3">
                  ...
                  {isBid ? (
                    <p className="text-xs text-gray-500">
                      This bid is saved for comparison only. Opsy will not
                      overwrite installed system details or property identity.
                    </p>
                  ) : (
                    <p className="text-xs text-gray-500">
                      Select the fields you want to apply
                      {identityFields.length
                        ? " to property identity and"
                        : " to"}{" "}
                      {systemLabel || "this system"} on the Extracted details
                      tab. Existing values are not overwritten unless you
                      choose them.
                      {identityFields.length > 0
                        ? " Invoice identity fields are preselected only when they are currently blank."
                        : ""}
                    </p>
                  )}
                  {isBid && onViewQuotes && (
              ...
              {activeTab === "details" && (
                <div className="space-y-2">
                  ...
                  {reviewFields.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      <p className="text-[11px] text-gray-500">
                        Quote details are saved for comparison and are not
                        written onto {systemLabel || "this system"}.
                      </p>
                      {(systemFields.length ? systemFields : reviewFields).map(
                        (field) => (
                          <div
                            key={field.fieldKey}
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <p className="text-[11px] text-gray-500 mt-0.5">
                              Blank fields are preselected. Existing identity
                              values stay unless you choose them.
                            </p>
                          </div>
                          {identityFields.map(renderReviewField)}
                        </div>
                      )}
                      {systemFields.length > 0 && (
                        <div className="space-y-2">
                          {identityFields.length > 0 && (
                            <h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500 pt-2">
                              System details
                            </h3>
                          )}
                          {systemFields.map(renderReviewField)}
                        </div>
                      )}
                    </>
                  )}
          ...
          <div className="flex-shrink-0 flex justify-end gap-2 px-5 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
            ...
            {isBid && onViewQuotes && (
              <button
                ...
                disabled={applying}
                onClick={onViewQuotes}
                className="btn bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 text-sm px-4 py-2 disabled:opacity-50 inline-flex items-center gap-1.5"
              >
                <ClipboardList className="w-3.5 h-3.5" />
                View quotes
              </button>
