...
const GOOGLE_SHEETS_TEMPLATE_URL =
  "https://docs.google.com/spreadsheets/d/1_TEMPLATE_PLACEHOLDER/edit";
...
function normalizeRow(rawRow, headerMap) {
  ...
  return CONTACT_IMPORT_KEYS.reduce((acc, k) => {
    acc[k] = row[k] ?? "";
    return acc;
  }, {});
}
...
function validateRow(row, rowIndex, emailSet) {
  const errors = [];
  const requiredKeys = CONTACT_IMPORT_FIELDS.filter((f) => f.required).map(
    (f) => f.key
  );
...
function buildWorkbookForDownload() {
  ...
  const ws = XLSX.utils.json_to_sheet([
    Object.fromEntries(
      CONTACT_IMPORT_FIELDS.map((f) => [f.label, row[f.key]])
    ),
  ]);
  ...
  const runImport = useCallback(() => {
    ...
    parseFile(fileToParse)
      .then(({ rows }) => {
        if (rows.length === 0) {
          setParseError(
            "No data rows found. Your file needs a header row (e.g. Name, Email) and at least one data row. " +
              "If you used our template, add contacts in the rows below the header and upload again."
          );
          setIsParsing(false);
          return;
        }
          ...
          <div className="px-0 sm:px-6 lg:px-8 xxl:px-16 py-6 w-full max-w-5xl mx-auto">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">
                Download a template, fill in your contacts, then upload and
              ...
              <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-800/30">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                    Use our template so column names match. Required: Name.
                  </p>
                </div>
                <div className="p-4 flex flex-wrap gap-3">
                  ...
                  <a
                    ...
                    className="btn border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-500 inline-flex items-center gap-2"
                  >
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-gray-50 dark:bg-gray-800/80">
                            ...
                            {CONTACT_IMPORT_FIELDS.slice(0, 6).map((f) => (
                              <th
                                key={f.key}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-3">
                      ...
                      >
                        {isSubmitting ? (
                          <>
                            <Loader2 className="w-4 h-4 animate-spin shrink-0" />
                            Creating {validCount} contact{validCount !== 1 ? "s" : ""}…
                          </>
                        ) : (
                          <>
                            Confirm import
                            {validCount > 0 && ` (${validCount} contact${validCount !== 1 ? "s" : ""})`}
                          </>
                        )}
                      </button>
