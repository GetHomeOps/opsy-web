...
import {X, Building, Zap, Loader2} from "lucide-react";
...
function CalendarScheduleModal({
  ...
  const {properties, getPropertyById, getSystemsByPropertyId} =
      ...
      .then(async (data) => {
        ...
        if (getSystemsByPropertyId) {
          try {
            ...
            if (!cancelled) {
              const systems = (systemsArr ?? []).filter(
                (s) => s.included !== false,
              );
            ...
            <div>
              ...
              >
                ...
                {(() => {
                  ...
                  const systemsToShow =
                    systemKeys.length > 0
                      ? PROPERTY_SYSTEMS.filter((sys) =>
                          systemKeys.includes(sys.id),
                        )
                      : [];
