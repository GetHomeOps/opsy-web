...
/** Maps form field keys to tab id (1–5) so validation errors can highlight the right tab. */
const PROFESSIONAL_FIELD_TAB = {
  company_name: 1,
  contact_name: 1,
  phone: 1,
  email: 1,
...
function ProfessionalFormContainer() {
  ...
  /** Bumped when save validation fails so tabs with errors replay the shake animation. */
  const [tabErrorShakeNonce, setTabErrorShakeNonce] = useState(0);

  const isNew = professionalId === "new" || !professionalId;

  const [professionalLoading, setProfessionalLoading] = useState(() => !isNew);
  ...
  const canUseTransientProfileImage =
    isNew || String(loadedProfessionalIdRef.current) === String(professionalId);
  ...
  const profileSrc = useMemo(() => {
    ...
    if (canUseTransientProfileImage && profilePreviewUrl)
      return profilePreviewUrl;
    if (canUseTransientProfileImage && profileUploadedUrl)
      return profileUploadedUrl;
  ...
  const tabIdsWithErrors = useMemo(() => {
    const s = new Set();
    for (const [key, msg] of Object.entries(state.errors)) {
      if (!msg) continue;
      const tid = PROFESSIONAL_FIELD_TAB[key];
      if (tid != null) s.add(tid);
    }
    return s;
  }, [state.errors]);
      ...
      <div className="relative flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
        ...
        <main className="grow">
          <div className="px-0 sm:px-4 lg:px-5 xxl:px-12 py-8 w-full max-w-5xl mx-auto">
            ...
            <div className="relative" aria-busy={professionalLoading}>
              {professionalLoading && (
                <div
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <div className="border-b border-gray-200 dark:border-gray-700">
                  <div className="px-6">
                    ...
                    >
                      {tabs.map((tab) => {
                        const hasTabError = tabIdsWithErrors.has(tab.id);
                        const isActive = state.activeTab === tab.id;
                        return (
                          <button
                            key={
                              hasTabError
                                ? `prof-tab-${tab.id}-${tabErrorShakeNonce}`
                                : `prof-tab-${tab.id}`
                            }
                            type="button"
                          ...
                          ${hasTabError ? "form-tab-error-shake" : ""}
                          ${
                            isActive && hasTabError
                              ? "border-red-500 text-red-600 dark:text-red-400"
                              : isActive
                                ? "border-[#456564] text-[#456564]"
                                : hasTabError
                                  ? "border-transparent text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:border-gray-300 dark:hover:border-gray-600"
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {LANGUAGE_OPTIONS.map((lang) => {
                              const active = (fd.languages || []).includes(
                                lang,
                              );
                              return (
                                <button
